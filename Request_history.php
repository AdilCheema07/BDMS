...
if ($userRole === "Admin") {
    // If user role is Admin, fetch all donations and requests with "approved" or "declined" status
    $donationSql = "SELECT username, age, gender, blood_quantity, blood_group, request_type, sent_at, status FROM donate_blood WHERE status IN ('fulfilled', 'cancelled')";
    $requestSql = "SELECT username, age, gender, blood_quantity, blood_group, reason, request_type, sent_at, status FROM request_blood WHERE status IN ('fulfilled', 'cancelled')";
} else {
    // If user role is not Admin, fetch donations and requests where usertype matches the userRole and status is "approved" or "declined"
    $donationSql = "SELECT username, age, gender, blood_quantity, blood_group, request_type, sent_at, status FROM donate_blood WHERE username = ? AND usertype = ? AND status IN ('fulfilled', 'cancelled')";
    $requestSql = "SELECT username, age, gender, blood_quantity, blood_group, reason, request_type, sent_at, status FROM request_blood WHERE username = ? AND usertype = ? AND status IN ('fulfilled', 'cancelled')";
}
...
if ($stmt) {
    if ($userRole === "Admin") {
        // If user role is Admin, there are no parameters to bind
    } else {
        // If user role is not Admin, bind both parameters (username and userRole)
