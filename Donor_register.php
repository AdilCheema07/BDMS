...
$username = $password = $confirm_password = $email = $cnic = $address = $age = $mobile = $gender = $blood_group = "";
$username_err = $password_err = $confirm_password_err = $email_err = $cnic_err = $address_err = $age_err = $mobile_err = "";
...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    if (empty(trim($_POST["username"]))) {
        ...
        $sql = "SELECT id FROM donors WHERE username = ?";
    ...
    if (empty(trim($_POST['password']))) {
        ...
        echo "<script>alert('Password must contain at least one uppercase letter, one lowercase letter, and one number');</script>";
    } else {
    ...
    if (empty(trim($_POST['age']))) {
        $age_err = "Age must be entered";
        echo "<script>alert('Age cannot be empty');</script>";
    } elseif (intval(trim($_POST['age'])) > 100) {
        $age_err = "You are too old to donate blood";
        echo "<script>alert('You are too old to donate blood');</script>";
    } elseif (intval(trim($_POST['age'])) < 0) {
        $age_err = "Age cannot be negative";
    ...
    // Store Blood Group Value (Do not need to check because user only have a few options to select from)
    ...
    if (empty($username_err) && empty($password_err) && empty($confirm_password_err) && empty($email_err) && empty($cnic_err) && empty($address_err) && empty($mobile_err) && empty($age_err)) {
        $sql = "INSERT INTO donors (username, password, email, cnic, address, age, mobile, gender, blood_group, usertype) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $stmt = mysqli_prepare($conn, $sql);

        if ($stmt) {
            mysqli_stmt_bind_param($stmt, "ssssssssss", $param_username, $param_password, $param_email, $param_cnic, $param_address, $param_age, $param_mobile, $param_gender, $param_blood_group, $param_usertype);
        ...
        <form action="" method="post">
            <div class="signup-box">
                ...
                <div class="form-box">
                    ...
                    <div class="form-box2">
                        <label for="">Age<sup class="red">*</sup>:</label>
                        <input name="age" type="number" class="input-box" placeholder="Enter Your Age" required="">
                    </div>
                    <br><br>
                ...
                <div class="form-box-submit">
                    ...
                    <span> <a id="signup" href="Donor_login.php">Sign in</a></span>
                </div>
        </form>
        </div>
    </section>
