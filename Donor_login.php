...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    if (empty($username_err) && empty($password_err)) {
        $sql = "SELECT id, username, password, age, blood_group, gender, usertype FROM donors WHERE username = ?";
        ...
        if (mysqli_stmt_execute($stmt)) {
            ...
            if (mysqli_stmt_num_rows($stmt) == 1) {
                mysqli_stmt_bind_result($stmt, $id, $username, $hased_password, $param_age, $param_blood_group, $param_gender, $param_usertype);
                if (mysqli_stmt_fetch($stmt)) {
                    if (password_verify($password, $hased_password)) {
                        ...
                        $_SESSION["blood_group"] = $param_blood_group;
...
<body>
    ...
    <section id="container">
        <div class="login-box">
            ...
            <br>
            <button type="submit" class="btn-login ">Login <svg fill="#ffffff" height="20px" width="20px"
            ...
            <span> <a id="signup" href="Donor_register.php">Sign up</a></span>
        </div>
        </form>
        </div>
    </section>
